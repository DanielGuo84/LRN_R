...
library(tidyverse)
library(readr)
setwd("C:/Users/Sabrina/riii/final")
```

```{r}
#(1) 請讀取final資料夾下的lvr_prices檔案 [5分]
load("C:/Users/Sabrina/riii/final/lvr_prices_big5.Rdata")
...
house <- lvr_prices[(lvr_prices$city_land_type == '住'& lvr_prices$total_price > 0 & lvr_prices$building_sqmeter >0 & as.numeric(lvr_prices$finish_ymd) != 1),]
...
house<- cbind.data.frame(house,price_per_sqmeter)
...
price_per_sqmeter = scale(price_per_sqmeter,center = TRUE,scale = TRUE)
house = house[abs(scale(house$price_per_sqmeter)) <= 3,]
